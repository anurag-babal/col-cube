...
string path = "/Users/sudhanshu/dm_project/COL-CUBE/FileStorage/bit/";
int cnt = 0;
map<string, map<string, string>> bitMap;
...
void doOperations(int index)
{
    ...
    int in = 0;
    // cerr<<fileNames[0]<<"\n";
    unordered_map<string, string> tempStorage;
    for (int i = 0; i < fileNames.size(); i++)
    {
        for (auto y : bitMap[fileNames[i]])
        {
            tempStorage[y.first] = y.second;
        }
    }
    ...
    for (int i = 0; i < 100000; i++)
    {
        ...
        if (comb.length() == 0)
            break;
        combString.push_back(comb);
    }

    for (int i = 0; i < combString.size(); i++)
    {
        for (int j = 0; j < facts.size(); j++)
        {
            for (int k = 0; k < aggregates.size(); k++)
            {
                doCalculations(combString[i], facts[j], aggregates[k], factsData[facts[j]][i]);
            }
        }
    }
    // cerr<<comb<<"\n";
...
int main()
{
    ...
    for (string &s : attributes)
    {
        ...
        string fileName = "/Users/sudhanshu/dm_project/COL-CUBE/cubestore/fact_sales/" + s + ".txt";
        ifstream inputFile(fileName);
        if (!inputFile.is_open())
        {
            cerr << "Error opening file: " << fileName << endl;
        ...
        string key, value;
        while (getline(inputFile, key) && getline(inputFile, value))
        {
            // cout << line << endl;  // Write each line to stdout
            bitMap[s][key] = value;
        }
